...
| SQL / Sample | 方言を切り替えると見本も入れ替わる（自分で書いた文は消えない） |
| SQL / VBA | `sql = sql & "…" & vbCrLf` 形式になる。`"` は `""` に。VBA を貼れば SQL に戻る |
| Excel / JSON | `001` は文字列、`7` は数値、`TRUE` は真偽値、空欄は `null` になる |
| Excel / Markdown | 数値列が右寄せ、セル内改行が `<br>`、`\|` がエスケープされる |
| Excel / CSV | カンマ区切りでも読める。`"Smith, J"` のようなクォート内カンマが壊れない |
| Excel / CSV 出力 | 引用符を含むセルが `"say ""hi"""` の形になる |
| Excel / pandas | `pd.DataFrame({...})` が出る。`\` は `\\`、空欄は `None`、TRUE は `True` |
